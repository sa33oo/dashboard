...
export default function Dashboard() {
  ...
  const agg = useMemo(() => {
    ...
    const byPayment = Object.entries(
      sales.reduce<Record<PaymentKey, { amount: number; count: number }>>(
        ...
        { 現金: { amount: 0, count: 0 }, カード: { amount: 0, count: 0 }, アプリ決済: { amount: 0, count: 0 }, 電子マネー: { amount: 0, count: 0 }, その他: { amount: 0, count: 0 } }
      )
    ).map(([name, v]) => ({ name, ...v }));

    const byDate = Object.entries(sales.reduce<Record<string, number>>((a, r) => ((a[r.date || '不明'] = (a[r.date || '不明'] || 0) + r.amount), a), {}))
      .map(([date, amount]) => ({ date, amount }))
      .sort((a, b) => a.date.localeCompare(b.date));

    const byItem = Object.entries(sales.reduce<Record<string, number>>((a, r) => ((a[r.itemName || '不明'] = (a[r.itemName || '不明'] || 0) + r.amount), a), {}))
  ...
  return <main className="p-6 space-y-6 max-w-7xl mx-auto">
    ...
    <div className="grid md:grid-cols-3 gap-4 bg-white p-4 rounded-xl shadow">
      ...
      <label>診療項目履歴Excel<input className="block mt-2" type="file" accept=".xlsx" onChange={(e) => handleUpload(e, 'items')} /></label>
      <label>飼い主・ペットExcel<input className="block mt-2" type="file" accept=".xlsx" onChange={(e) => handleUpload(e, 'pets')} /></label>
    </div>
    <p className="text-sm text-slate-600">読込件数: 売上 {sales.length}件 / ペット {pets.length}件 / 診療項目履歴 {itemsLoaded ? '読込済み' : '未読込'}</p>
